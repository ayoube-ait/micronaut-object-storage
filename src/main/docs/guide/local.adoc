...
The local implementation also implements api:objectstorage.CommittingObjectStorageOperations[] and supports its
`uploadAndCommit(UploadRequest, UploadCommitter)` operation. It invokes the api:objectstorage.UploadCommitter[] exactly
once, synchronously, after the new bytes are durable and the ETag, actual stored content length, content type, and storage
timestamp are finalized, but before releasing the per-object mutation lock or deleting the rollback snapshot. It does
not invoke the callback if uploading the bytes fails, and it does not retry a failed callback. Callback code must not
re-enter storage operations for the same physical object key.

The callback receives api:objectstorage.CompletedUpload[], owns application metadata persistence and its transaction for
this operation, and can return an application-defined result in api:objectstorage.CommittedUpload[]. A successful return
publishes the upload. A thrown exception means publication failed and starts compensation. Unlike the existing `upload`
methods, `uploadAndCommit` does not call api:objectstorage.metadata.ObjectMetadataOperations[] and does not create or
modify Micronaut object metadata sidecars. The callback's authoritative metadata store therefore does not need to be
exposed through the Micronaut metadata SPI.

If the callback fails for a new object, the local provider removes the uploaded bytes. If it fails while replacing an
object, the provider restores the previous bytes; provider metadata sidecars remain untouched throughout the operation.
The callback transaction is responsible for rolling back application-owned metadata. A rollback failure is attached to
the callback failure as a suppressed exception. Once the callback returns successfully, the local provider never rolls
back the uploaded bytes, and a subsequent rollback-snapshot cleanup failure does not turn the successful operation into
a failure. These are in-process compensation guarantees, not a crash-atomic transaction with an external metadata
store. Other providers expose this operation only if they implement api:objectstorage.CommittingObjectStorageOperations[]
and may document different provider-specific compensation guarantees.
